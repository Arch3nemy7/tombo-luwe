    ...
    <div class="container-fluid">
        <div class="row-checkout">
            ...
            if (isset($_SESSION["uid"])) {
                ...
                echo '<div class="col-75">
                    ...
                    while ($i <= $total_count) {
                        ...
                        $item_name_ = $_POST['product_title_' . $i];
                        $amount_ = $_POST['product_price_' . $i];
                        $quantity_ = $_POST['qty_' . $i];
                        $total += $amount_ * $quantity_; // Calculate total for order
        
                        echo '<input type="hidden" name="product_id[]" value="' . $product_id . '">';
                        echo '<input type="hidden" name="product_qty_' . $product_id . '" value="' . $quantity_ . '">';
                        echo '<input type="hidden" name="product_price_' . $product_id . '" value="' . $amount_ . '">';
                    ...
                    echo '<input type="submit" id="submit" value="Continue to checkout" class="checkout-btn">';
                    echo '</form>';
                    echo '</div>';
                    echo '</div>';
                } else {
                    echo "<script>window.location.href = 'cart.php'</script>"; // Redirect if not logged in
                }
            ?>
            ...
            <div class="col-25">
                <div class="container-checkout">
                    <?php
                    // Check if coming from cart.php
                    if (isset($_POST['cmd'])) {
                        // Retrieve cart data from $_POST
                        ...
                        while ($i <= $total_count) {
                            ...
                            $total += $amount_ * $quantity_; // Calculate subtotal for each item
                            ...
                            // Pass product IDs as hidden inputs for checkout_process.php
                            echo "<input type='hidden' name='product_id[]' value='$product_id'>";
                            echo "<input type='hidden' name='product_qty_$product_id' value='$quantity_'>";
                            echo "<input type='hidden' name='product_price_$product_id' value='$amount_'>";
