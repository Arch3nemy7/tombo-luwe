...
if (isset($_POST["count_item"])) {
    // When user is logged in, count the number of items in the cart using user session id
    if (isset($_SESSION["uid"])) {
        $sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
    } else {
        // When user is not logged in, count the number of items using the user's unique IP address
...
if (isset($_POST["Common"])) {
    if (isset($_SESSION["uid"])) {
        // When user is logged in, execute this query
        $sql = "SELECT a.product_id, a.product_title, a.product_price, a.product_image, b.id, b.qty FROM products a, cart b WHERE a.product_id = b.p_id AND b.user_id = '$_SESSION[uid]'";
    } else {
        // When user is not logged in, execute this query
    ...
    if (isset($_POST["checkOutDetails"])) {
        if (pg_num_rows($query) > 0) {
            ...
            if (!isset($_SESSION["uid"])) {
                // Condition 1: User is not logged in, redirect to login_form
                echo '<div class="text-center">
                        <a href="login_form.php" class="btn btn-success">Log In to Checkout</a>
                      </div>';
            } else {
                // Condition 2: User is logged in, display checkout form
                echo '<form action="checkout_process.php" method="post">
                        <!-- Add necessary inputs and checkout details -->
...
//     if (isset($_POST["checkOutDetails"])) {
//         if (pg_num_rows($query) > 0) {
//             // Display user cart items with "Ready to checkout" button if user is not logged in
//             echo '<div class="main">
//                     <div class="table-responsive">
...
//                 echo '<input type="hidden" name="return" value="http://localhost/myfiles/public_html/payment_success.php"/>
